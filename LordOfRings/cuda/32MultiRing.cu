...
__device__ inline int Ptolemy ( const float& x , const float& y , 
                               const float * triplet , const float ptolemy_threshold)
{
    ...
    if (ptoval < ptolemy_threshold) return 1;
    else return 0;
}

__global__ void multiring( float * x, float * y , int * triplet, // coordinates and triplets
                           float ptolemy_threshold, // threshold on ptolemy theorem
			               ...
			               float rsearch, float drsearch)
{
    ...
    if ( Ptolemy ( x[ hitIdx ] , y[ hitIdx ], triplet_s , ptolemy_threshold) && ( x [ hitIdx ] != 0) && ( y[ hitIdx ] != 0) ) {
    ...
    if (length < MAXHITS/8){    //there are less than 8 points (not enough accurate fit)
        ...
        if (threadIdx.x == 0) {
            if (( radius [ blockIdx.x ] <= rsearch + drsearch ) && (  radius [ blockIdx.x ] >= rsearch - drsearch )){
                radius [ blockIdx.x ] = -1* radius [blockIdx.x];
                }
            }
        }
    }
